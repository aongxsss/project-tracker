              ...
              {affectedProjects.map((p) => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "#F9F8F5", borderRadius: 8, border: "1px solid #F0EEE8" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: "#999", marginTop: 1 }}>{p.pm} · Due {formatDate(p.due_date)}</div>
                  </div>
                  <span style={{ fontSize: 11, color: "#888", background: "#EFEFEF", borderRadius: 999, padding: "2px 8px", whiteSpace: "nowrap", flexShrink: 0 }}>{p.status}</span>
                </div>
              ))}
            </div>
          </div>
